...
void strips_boundary_exchange(world *cur_world , int *start_strip, int *end_strip, int myrank, int MPIsize, int world_rows, int world_cols )
{
...
main(int argc, char *argv[])
{
    ...
    for (world_iter = 1; world_iter < nsteps; world_iter++) {
        ...
        if(size > 1)
            strips_boundary_exchange( cur_world, start_strip, end_strip, rank, size, world_rows, world_cols );
        else world_border_wrap(cur_world);
