...
double strips_boundary_exchange(world *cur_world , int *start_strip, int *end_strip, int myrank, int MPIsize, int world_rows, int world_cols )
{
    ...
    double t1 = time_secs(), sync_time = 0.0;

    /* odd nodes */
    if ( myrank % 2 != 0 )
    {
        if ( myrank < MPIsize-1 )  /* send to right - top */
            MPI_Send( &matrix[end_strip[myrank]][0], world_cols + 2, MPI_INT, top_process, 102, MPI_COMM_WORLD );
        if ( myrank > 0 )          /* recv from left - bottom */
            MPI_Recv( &matrix[start_strip[myrank]-1][0], world_cols + 2, MPI_INT, bot_process, 101, MPI_COMM_WORLD,&recv_status );
        if ( myrank > 0 )          /* send to left - bottom */
            MPI_Send( &matrix[start_strip[myrank]][0], world_cols + 2, MPI_INT, bot_process, 104, MPI_COMM_WORLD );
        if ( myrank < MPIsize-1 )  /* recv from right - top */
            MPI_Recv( &matrix[end_strip[myrank]+1][0], world_cols + 2, MPI_INT, top_process, 103, MPI_COMM_WORLD, &recv_status );
    }
    /* even nodes */
    else
    {	      
        if ( myrank > 1 )           /* recv from left - bottom */
            MPI_Recv( &matrix[start_strip[myrank]-1][0], world_cols+2, MPI_INT, bot_process, 102, MPI_COMM_WORLD, &recv_status );
        if ( myrank < MPIsize-1 )   /* send to right - top */
        {
            sync_time += time_secs() - t1;
            MPI_Send( &matrix[end_strip[myrank]][0], world_cols+2, MPI_INT, top_process, 101, MPI_COMM_WORLD );
        }    
        if ( myrank < MPIsize-1 )   /* recv from right - top */
        {
            MPI_Recv( &matrix[end_strip[myrank]+1][0], world_cols+2, MPI_INT, top_process, 104, MPI_COMM_WORLD, &recv_status );
            t1 = time_secs();
        }

        if ( myrank > 1 )           /* send to left - bottom */
            MPI_Send( &matrix[start_strip[myrank]][0], world_cols+2, MPI_INT, bot_process, 103, MPI_COMM_WORLD );
    }

    /*upper and lower exchange*/
    if ( myrank == 0 ){
        sync_time += time_secs() - t1;
        MPI_Send( &matrix[1][0], world_cols+2, MPI_INT, MPIsize-1, 105, MPI_COMM_WORLD );
        MPI_Recv( &matrix[world_rows][0], world_cols+2, MPI_INT, MPIsize-1, 106, MPI_COMM_WORLD, &recv_status );
        t1 = time_secs();
    }
    if ( myrank == MPIsize-1 ){
        MPI_Recv( &matrix[1][0], world_cols+2, MPI_INT, 0, 105, MPI_COMM_WORLD, &recv_status );
        MPI_Send( &matrix[world_rows][0], world_cols+2, MPI_INT, 0, 106, MPI_COMM_WORLD );
    }
    sync_time += time_secs() - t1;
    return sync_time;
}
...
main(int argc, char *argv[])
{
    ...
    double comm_time = 0.0, sync_time = 0.0, comp_time = 0.0;
    int i, j;
    int *transfer;
    ...
    for (world_iter = 1; world_iter < nsteps; world_iter++) {
        ...
        t1 = time_secs();
        world_border_wrap(cur_world);
        world_timestep( cur_world, next_world, start_strip[rank], end_strip[rank], world_rows, world_cols );
        
        /* swap old strip content for new generation */
        cur_world = next_world;
        comp_time += time_secs() - t1;

        /* internal boundaries between strips */
        /* real boundary is not affected */
        t1 = time_secs();
        if(size > 1)
            sync_time += strips_boundary_exchange( cur_world, start_strip, end_strip, rank, size, world_rows, world_cols );
        comm_time += time_secs() - t1;
        ...
        if ( rank == 0 )
        {
            ...
            if (total_stop_signal == size)
            {
                ...
                printf("Synchronization time = %10.7f seconds\n", sync_time);
                printf("Communication time = %10.3f seconds\n", comm_time-sync_time);
                printf("Computation time = %10.3f seconds\n", comp_time);
