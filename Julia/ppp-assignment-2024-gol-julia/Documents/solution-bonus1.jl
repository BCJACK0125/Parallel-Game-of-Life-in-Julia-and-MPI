...
function game_serial_impl(chnl_anim,fn,params)
    ...
    for istep in 1:steps
        computation_time += step_serial!(a_new,a)
        tmp = a
        a = a_new
        a_new = a_history[2]
    ...
    println("Serial computation time: $computation_time seconds")
...
function game_worker(I,J,fn,params,channels)
    ...
    comm_time = 0.0
    for istep in 1:steps
        comm_time += step_worker!(a_new, a, chnls_snd, chnls_rcv)
        tmp = a
        a = a_new
        a_new = a_history[2]
        a_history[2] = a_history[1]
    ...
    wall_time = time() - wall_time
    println("Computation time: $(comm_time) seconds")
    return wall_time
end
