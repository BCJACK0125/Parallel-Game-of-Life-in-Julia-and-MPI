...
for filename in filenames:    
    ...
    if count % 4 == 0:
        y_axis_64000.append(((sum(serial_64000)/len(serial_64000))/min(value))/len(data["wall_time"]))
        # print(min(value))
        value = []
# print(y_axis_64000)
...
for filename in filenames:    
    ...
    # Do something with the value
    if count % 4 == 0:
        y_axis_32000.append(((sum(serial_32000)/len(serial_32000))/min(value))/len(data["wall_time"]))
        # print(min(value))
        value = []
print(y_axis_32000)

x = np.array(range(1, 130, 3))
y = 1.5/np.sqrt(x) # Communication
...
# Plotting x and y with blue line and points
plt.plot(x, y, '--', color='blue')  # '-o' specifies the line style as a solid line with points and color='blue' sets the line color to blue
...
plt.legend(['m=n=64000', 'm=n=32000', r'Derivative of  $\frac{\sqrt{P}}{N}$ ($\frac{Communication}{Computation}$)'], loc='upper right')
